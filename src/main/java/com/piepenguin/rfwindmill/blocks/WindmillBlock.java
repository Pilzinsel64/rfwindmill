...
public class WindmillBlock extends Block implements ITileEntityProvider {
    ...
    public WindmillBlock(String pName, float[] pEfficiency, int[] pCapacity) {
        ...
        Arrays.fill(
            ...
            (int) (ModConfiguration.getWindGenerationBase() * ModConfiguration.getWindmillEnergyTransferMultiplier()));
        capacity = pCapacity;
        name = pName;
    ...
    @Override
    public boolean onBlockActivated(World pWorld, int pX, int pY, int pZ, EntityPlayer pPlayer, int pSide, float pDx,
        float pDy, float pDz) {
        if (!pWorld.isRemote) {
            if (pPlayer.isSneaking()) {
                ...
                if (equippedItem != null
                    && (equippedItem.getItem() == ModItems.rotor1 || equippedItem.getItem() == ModItems.rotor2
    ...
    private void dismantle(World pWorld, int pX, int pY, int pZ) {
        ...
        if (entity.hasRotor()) {
            ...
            RotorBlock rotor = (RotorBlock) pWorld.getBlock(pX + dir.offsetX, pY + dir.offsetY, pZ + dir.offsetZ);
        ...
        if (energy > 0) {
            ...
            itemStack.getTagCompound()
                .setInteger(EnergyStorage.NBT_ENERGY, energy);
        }
