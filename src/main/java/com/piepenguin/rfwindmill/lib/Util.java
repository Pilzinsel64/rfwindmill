...
public class Util {
    ...
    public static boolean hasWrench(EntityPlayer pPlayer, int pX, int pY, int pZ) {
        ...
        return tool != null
            && (ModAPIManager.INSTANCE.hasAPI("BuildCraftAPI|tools") && (tool.getItem() instanceof IToolWrench)
    ...
    public static int directionToInt(ForgeDirection pDir) {
        if (pDir == ForgeDirection.NORTH) return 0;
        else if (pDir == ForgeDirection.EAST) return 1;
        else if (pDir == ForgeDirection.SOUTH) return 2;
        else if (pDir == ForgeDirection.WEST) return 3;
        else return 0;
    }
